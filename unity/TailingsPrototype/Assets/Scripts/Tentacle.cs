...
namespace SplineMesh {
    ...
    public class Tentacle : MonoBehaviour {
		...
		private void Update() {
            for (int i=spline.nodes.Count-1; i>1; i--) {
                spline.RemoveNode(spline.nodes[i]);
			}
			for (int i=0; i<5; i++) {
                float x = UnityEngine.Random.Range(-i, i);
                float y = UnityEngine.Random.Range(-i, i);
                float z = UnityEngine.Random.Range(-i, i);
                SplineNode node = new SplineNode(new Vector3(x, y, z), new Vector3(0, 1, 0));
                spline.AddNode(node);
            }
		}
